...
export const Modal = () => {
  ...
  useEffect(() => {
    setValid(inputText !== '' && file !== undefined && user !== undefined && user !== null);
  }, [showModal, inputText, user, file]);
  ...
  const add = async () => {
    ...
    try {
      ...
      // Reset states after entry is made
      setFile(undefined);
      setInputText('');
  ...
  return (
    <>
      ...
      "
                rows={7}
                placeholder="Enter name of certificate or document"
                onChange={(e) => {
